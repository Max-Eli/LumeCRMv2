...
class Message(TenantedModel):
    ...
    # AI-inbox fields (Phase 1). All optional / default-falsy so the
    # row shape is unchanged for messages that have nothing to do
    # with the AI agent (manual sends, automated reminders, etc.).
    generated_by_ai = models.BooleanField(
        default=False,
        db_index=True,
        help_text=(
            'True for outbound rows the AI agent wrote. Drives the '
            "purple bubble + AI pill in the inbox UI. Never True on "
            'inbound rows (the AI does not author inbound messages).'
        ),
    )
    ai_conversation = models.ForeignKey(
        ...
        related_name='messages',
        help_text=(
            'Links the message to its AI-agent conversation row. '
            'Set on both inbound (when dispatched to the AI) and '
            'outbound (when the AI wrote the reply). SET_NULL so '
            'closing or deleting an AIConversation does not break '
            'the messaging audit trail.'
        ),
    )
    parent_inbound_message_id = models.PositiveBigIntegerField(
        null=True, blank=True, db_index=True,
        help_text=(
            'For outbound AI replies: the ID of the inbound Message '
            'that triggered this turn. Drives per-inbound send '
            'idempotency — if the same inbound is processed twice '
            '(Twilio retry, double-fire), the second pass becomes a '
            'no-op because a row with this parent already exists.'
        ),
    )
