...
- [x] **SES bounce/complaint → SNS pipeline** — shipped 2026-05-17 ([ADR 0029](docs/decisions/0029-ses-bounce-complaint-suppression.md)). `infra/email.tf` provisions the SES configuration set, SNS topic, HTTPS subscription, IAM. Django receiver at `/api/aws/ses-events/` verifies AWS's X.509 signature, auto-confirms the subscription, upserts `EmailSuppression` rows on permanent bounces + complaints (idempotent on repeat events), and ALWAYS returns 200 (per ADR 0027 §3 — never 4xx the provider). `SuppressionCheckingSESBackend` wraps `django_ses.SESBackend` and drops suppressed recipients from `to` / `cc` / `bcc` before forwarding — every existing AND future `EmailMultiAlternatives` callsite is now policy-gated with zero callsite refactor. 23 tests covering signature pattern guard, Gmail +tag normalisation, permanent vs transient bounce dispatch, complaint → suppression, repeat-event idempotency, backend partial-drop, subscription-confirm round-trip, and robustness (bad JSON / bad sig / unknown event type all return 200).
- [x] **SES configuration set on every outbound message** — shipped 2026-05-17 (same ADR). `AWS_SES_CONFIGURATION_SET` env var set on the backend task definition from the Terraform-managed config-set name; `django-ses` attaches it to every SendEmail call automatically. Without this, none of the bounce/complaint events would publish.
...
- [x] **CloudWatch alarms on SES bounce + complaint rates** — shipped 2026-05-17 (same ADR). Four alarms onto the existing `aws_sns_topic.alarms` topic: bounce rate >3% (warn) / >5% (critical), complaint rate >0.1% (warn) / >0.3% (critical). 15-min evaluation period matching SES's reputation-reporting cadence.
